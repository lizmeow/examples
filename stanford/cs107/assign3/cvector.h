 ...
 * an assert is raised.
 *
 * The elemsz parameter specifies the size, in bytes, of the elements
 ...
 * raised if elemsz is not greater than zero.
 *
 ...
 * If planning to store many elements, specifying a large capacity_hint will
 ...
 * is less than 0 or greater than the count. addr is expected to be
 * a valid pointer and the new element is copied from the memory pointed to
 * by addr. The capacity is enlarged if necessary, an assert is raised
 ...
 * ----------------------------
 * Appends a new element to the end of the CVector. addr is expected to 
 * be a valid pointer and the new element is copied from the memory pointed 
 * to by addr. The capacity is enlarged if necessary, an assert is raised 
 ...
 * element. addr is expected to be a valid pointer and the new element 
 * is copied from the memory pointed to by addr and replaces the 
 * old element at index. An assert is raised if index is out of bounds. 
 * Operates in constant-time.
 */
 ...
 * given key points. It uses the provided cmp callback to compare
 * elements. The search considers all elements from start index to end. 
 ...
 * multiple iterations simultaneously without interference, however, the client
