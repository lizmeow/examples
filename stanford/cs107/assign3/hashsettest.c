...
static void TestHashTable(void)
{
  ...
  hashset_new(&counts, sizeof(struct frequency), kNumBuckets, HashFrequency, CompareLetter, NULL);
  
  fprintf(stdout, "\n\n ------------------------- Starting the HashTable test\n");
  BuildTableOfLetterCounts(&counts);
  
  fprintf(stdout, "Here is the unordered contents of the table:\n");
  hashset_map(&counts, PrintFrequency, stdout);  // print contents of table
  
  vector_new(&sortedCounts, sizeof(struct frequency), NULL, 0);
  hashset_map(&counts, AddFrequency, &sortedCounts);   // add all freq to array
  vector_sort(&sortedCounts, CompareLetter);      // sort by char
  fprintf(stdout, "\nHere are the trials sorted by char: \n");
  vector_map(&sortedCounts, PrintFrequency, stdout);
  
  vector_sort(&sortedCounts, CompareOccurrences); //sort by occurrences
  fprintf(stdout, "\nHere are the trials sorted by occurrence & char: \n");
  vector_map(&sortedCounts, PrintFrequency, stdout);	// print out array 
