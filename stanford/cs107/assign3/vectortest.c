...
static void TestSearch(vector *v, char ch)
{
  ...
  foundSorted = vector_search(v, &ch, CompareChar, 0, true); // Test sorted 
  foundNot = vector_search(v, &ch, CompareChar, 0, false);   // Not sorted 
...
static void TestInsertDelete(vector *alphabet)
{
  ...
  for (i = 3; i < vector_length(alphabet); i += 3) // Delete every 4th char 
    vector_elem_delete(alphabet, i);
  fprintf(stdout, "\nAfter deleting dashes: ");
  vector_map(alphabet, PrintChar, stdout);
    
  ch = '!';
  vector_insert(alphabet, &ch, vector_length(alphabet));
  vector_elem_delete(alphabet, vector_length(alphabet) - 1);
...
static void TestReplace(vector *alphabet)
{
  ...
  while (found < vector_length(alphabet)) {
    found = vector_search(alphabet, &toFind, CompareChar, found, false);
    if (found == -1) break;
    vector_elem_replace(alphabet, &toReplace, found);
  }
 ...
 * ------------------------------------
 * Empties out the vector in such a way that vector_elem_delete
...
static void DeleteEverythingVerySlowly(vector *numbers)
{
  ...
  while (vector_length(numbers) >= 100) {
    vector_elem_delete(numbers, vector_length(numbers) - 100);
  ...
  fflush(stdout);
  while (vector_length(numbers) > 0) vector_elem_delete(numbers, 0);
